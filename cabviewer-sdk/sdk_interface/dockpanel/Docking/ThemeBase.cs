...
namespace CabViewerSDK.Docking
{
    public abstract class ThemeBase : Component
    {
        ...
        public void ApplyTo(DockPanel dockPanel)
        {
            ...
            {
                throw new InvalidOperationException(CabViewerSDK.Properties.DockingStrings.Theme_MissingFactory);
            }

            if (dockPanel.Panes.Count > 0)
                throw new InvalidOperationException(CabViewerSDK.Properties.DockingStrings.Theme_PaneNotClosed);

            if (dockPanel.FloatWindows.Count > 0)
                throw new InvalidOperationException(CabViewerSDK.Properties.DockingStrings.Theme_FloatWindowNotClosed);

            if (dockPanel.Contents.Count > 0)
                throw new InvalidOperationException(CabViewerSDK.Properties.DockingStrings.Theme_DockContentNotClosed);

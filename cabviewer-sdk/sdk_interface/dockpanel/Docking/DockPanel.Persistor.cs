    ...
    partial class DockPanel
    {
        private static class Persistor
        {
            ...
            public static void SaveAsXml(DockPanel dockPanel, Stream stream, Encoding encoding, bool upstream)
            {
                ...
                xmlOut.WriteComment(CabViewerSDK.Properties.DockingStrings.DockPanel_Persistor_XmlFileComment1);
                xmlOut.WriteComment(CabViewerSDK.Properties.DockingStrings.DockPanel_Persistor_XmlFileComment2);
            ...
            private static ContentStruct[] LoadContents(XmlTextReader xmlIn)
            {
                ...
                for (int i = 0; i < countOfContents; i++)
                {
                    ...
                    if (xmlIn.Name != "Content" || id != i)
                        throw new ArgumentException(CabViewerSDK.Properties.DockingStrings.DockPanel_LoadFromXml_InvalidXmlFormat);
            ...
            public static void LoadFromXml(DockPanel dockPanel, Stream stream, DeserializeDockContent deserializeContent, bool closeStream)
            {
                if (dockPanel.Contents.Count != 0)
                    throw new InvalidOperationException(CabViewerSDK.Properties.DockingStrings.DockPanel_LoadFromXml_AlreadyInitialized);
                ...
                using (var xmlIn = new XmlTextReader(stream) { WhitespaceHandling = WhitespaceHandling.None })
                {
                    ...
                    if (!IsFormatVersionValid(formatVersion))
                        throw new ArgumentException(CabViewerSDK.Properties.DockingStrings.DockPanel_LoadFromXml_InvalidFormatVersion);
